...
beers_brewery_merge_style %>% group_by(brew_style) %>% dplyr::summarize(count=n()) %>% mutate(percent=(count/sum(count))*100) %>%
  ggplot() + geom_bar(aes(y=percent,x=brew_style, fill=brew_style), stat = "identity") +  theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + scale_fill_manual(values= solarized_pal()(3))
