...
state_count %>% ggplot() + geom_bar(aes(y=count,x=reorder(State, -count)), stat = "identity",fill = "#cb4b16") + theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + theme(axis.text.x = element_text(angle = 90)) + labs(x="State", y="Count", title="Breweries by State")
```
...
ABV_STATE %>% ggplot(aes(x=reorder(State, -ABV), y=ABV))  +
  geom_bar(stat="identity",fill = "#cb4b16") + theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + theme(axis.text.x = element_text(angle = 90)) + labs(title ="ABV By State", x= "State", y = "Acohol By Volume" )
...
IBU_STATE %>% ggplot(aes(x=reorder(State, -IBU), y=IBU)) +
  geom_bar(stat="identity", fill = "#cb4b16") + theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + theme(axis.text.x = element_text(angle = 90)) + labs(title ="IBU By State", x= "State", y = "Internal Bitterness Units" )
...
beers_with_abv %>% ggplot(aes(x=ABV)) + geom_histogram(fill = "#cb4b16")  + theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + labs(title="Distribution of beers by ABV", x="Alcohol By Volume", y = "N")
...
beers_brewery_merge_style %>% group_by(brew_style) %>% dplyr::summarize(count=n()) %>% mutate(percent=(count/sum(count))*100) %>%
  ggplot() + geom_bar(aes(y=percent,x=brew_style, fill=brew_style), stat = "identity") +  theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + scale_color_manual(values= solarized_pal()(3))
...
na.omit(beers_brewery_merge_style) %>%  ggplot(aes(x = ABV, y = IBU, color=brew_style)) + geom_point() + geom_smooth(method = "lm")  +  theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') +  labs(title ="ABV vs. IBU")
...
beers_brewery_merge_style %>% ggplot(aes(x=ABV)) + geom_histogram(fill = "#cb4b16")  +  theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + facet_wrap(~brew_style)
...
knn_result_df = data.frame(seq(1,50,1),MeanAcc)
 knn_result_df %>% ggplot(aes(x=knn_result_df$seq.1..50..1.,y=knn_result_df$MeanAcc)) + geom_line(stat = "identity", color="#cb4b16")  +  theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') + labs(title="Accuracy vs. K", x="K Size", y="Accuracy")
...
data_with_imputed_ibus  %>%
  ...
  ggplot(aes(ABV, IBU_imputed, color = imputed_ind)) +
    ...
    geom_point() +
    facet_wrap(~brew_style) + theme_solarized_2(light=FALSE) + scale_colour_solarized('blue') +  
    labs(x="ABV", 
         y="IBU",
...
data_with_imputed_ibus %>%
 group_by(State) %>%
  ...
  ggplot(aes(reorder(State, -med_IBU, FUN="median"), med_IBU)) +
    ...
    geom_bar(stat = "identity", fill = "#cb4b16") + theme_solarized_2(light=FALSE) + scale_colour_solarized('blue')  +  labs(x="States", 
         y="Median IBU",
