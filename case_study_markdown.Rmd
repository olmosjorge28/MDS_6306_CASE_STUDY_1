...
omitted_nas <- na.omit(beers_brewery_merge_style)
...
knn_test <- function(data,columns,classifications,ratio, iterations, numks){
 ...
 head(omitted_nas)
 MeanAcc = colMeans(knn_test(omitted_nas,c("IBU","ABV"),c("brew_style"), .7, 1000, 50))
 ...
 ipa_ales_only <- omitted_nas[omitted_nas$brew_style!="Other",]
...
prediction(omitted_nas,c(4,5),c("brew_style"), .7, 100)
```


## Summarizing by brewery


```{r}

brewery_size <-  Vectorize(function(count){
  size="L"
  if(count<10){
    size = "S"
...
brewery_size(20)
beers_brewery_merge_style %>% group_by(brewery_name) %>% dplyr::summarize(count=n()) %>% mutate(brewery_size=brewery_size(count)) 

beers_brewery_merge_style %>% group_by(brewery_name) %>% dplyr::summarize(count=n()) %>% mutate(brewery_size=brewery_size(count)) %>% ggplot() + geom_bar(aes(y=count,x=brewery_size, fill=brewery_size), stat = "identity") 


brewer_size_data <- beers_brewery_merge_style %>% group_by(brewery_name) %>% dplyr::summarize(count=n()) %>% mutate(brewery_size=brewery_size(count)) 

merged_data_with_brewery_size = merge(beers_brewery_merge_style,brewer_size_data,"brewery_name")


na.omit(merged_data_with_brewery_size) %>%  ggplot(aes(x = ABV, y = IBU, color=brewery_size)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~brew_style) +  labs(title ="ABV vs. IBU")


merged_data_with_brewery_size %>%  ggplot(aes( y = IBU, color=brewery_size)) + geom_boxplot() + facet_wrap(~brew_style)


na.omit(merged_data_with_brewery_size) %>%  ggplot(aes( y = ABV, color=brewery_size)) + geom_boxplot() + facet_wrap(~brew_style)
...
plot_usmap(data = state_count, values = State, color = "red") + 
